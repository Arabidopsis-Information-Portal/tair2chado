...
$SQL = sprintf ("SELECT * FROM (SELECT tairObjectId, germplasmTairObjectId, name, speciesVariantAbbrevName, polymorphismCount, stockCount, imageCount, phenotypeCount FROM aip.LocusGermplasm ORDER BY phenotypeCount + imageCount + stockCount DESC, polymorphismCount DESC) ");
